...
**Version 1.33** · Copies of this appear inside `OFFICE.md` and `SYSTEM.md` for reading convenience. **If they ever disagree, this file wins.**
...
type AgentView = {
  ...
  role: AgentRole              // the office CATEGORY — which room-group the sprite stands in
  roleId: string | null        // ★ 1.28 the role DEFINITION it is briefed from, e.g.
                               //   "security-auditor". arbitrary; null = built-in brief
  ...
  allowTools: string[] | null  // ★ 1.29 tools this agent may use; null = machine defaults
  denyPaths: string[] | null   // ★ 1.29 paths it may not touch; null = machine defaults
...
| Version | Change | Why |
|---|---|---|
| **1.33** | `BoardTask.gate` — **what "done" means for a task, and how far it has got**. Optional; absent on an ungated task. Carries `outputs` (declared kinds and whether each is on record), `checks` (named acceptance checks with `passed`/`failed`/`skipped`/not-run and the server's own reason), `review` (`pending`/`accepted`/`rejected`/`unavailable`/`abandoned` plus the reviewer's name), `blockedOn`, and `unverified` | Three gates were built (1.30, 1.31, 1.32), tested, and **invisible**. `BoardTask` shipped eight fields and not one said why a task was standing still, so a reviewer could hold work and the office would show an agent doing nothing with no reason given — failing the product's own test, that a stranger can watch for 60 seconds and say what the team is doing. Four deliberate choices. **Absent, not empty**, when nothing gates a task: an empty object would claim it was gated and passed. **Read-only** — the office renders this and never decides from it; the decision stays in `completionGate.ts`. **Only a gate that has spoken blocks**: a task still being worked on has produced nothing yet, and reporting that as "missing output" would paint half the board red, so `blockedOn` is set only where a gate event proves the refusal. And a check name nobody defined reads as **failed, not skipped**, because `runAcceptanceChecks()` records it `passed:false` and the gate genuinely refuses the completion — only a workspace on another machine is a true skip. `POST /api/tasks` now also reads `expectedOutputs`, `acceptanceChecks` and `requiresReview`, which `createTask()` had accepted all along: until now every task a **human** made bypassed all three gates while planner-made tasks were held to them |
| **1.32** | No view change — **review is a state work passes through**: a task marked `requires_review` cannot reach `completed` until a reviewer's newest verdict is ACCEPT | `processReviewResult()` was already a whole rework lifecycle — ACCEPT completes, REJECT opens a linked rework task and escalates after N attempts — reachable only through an HTTP route somebody had to call. Nothing ever REQUIRED a review, so work could be written, claimed done and completed with no reviewer ever seeing it: the reviewer role existed, being reviewed did not. The check joins the completion gate as its third question, after artifacts (1.30) and acceptance checks (1.31), and is asked only once those pass — there is no point sending a reviewer at output that does not exist or tests that fail. **No new `TaskState`**: a held task simply does not complete, exactly as with a missing artifact, so the enum and the renderer are untouched. The review is dispatched as a real task, so it is scheduled, on the board, and under the same budget and lease machinery as any other work. Three deliberate refusals: the author never reviews their own work; a non-reviewer is **not** accepted as a fallback, because "anyone who is free" reviewing a diff is a rubber stamp and a rubber stamp is worse than an honest gap; and the NEWEST verdict decides, so an ACCEPT followed by more work and a REJECT does not still read as accepted. It cannot deadlock — after `MAX_REVIEW_CYCLES` the completion is allowed through and recorded as `task.completed_unverified` |
| **1.31** | No view change — **acceptance checks**: a project defines named commands, and a task may require them to exit 0 before it reaches `completed` | The completion gate (1.30) proves an output EXISTS; it cannot prove the output is RIGHT. An exit code can, and costs no model tokens — the evidence is the process, not another opinion. **The command is stored per project and a task may only ever reference one BY NAME.** That is the whole safety argument: tasks are built from agent-authored content in places (`plan-proposals.ts` lifts `title` and `capability` straight out of a plan a model wrote), so a task field holding a shell string would be a path from "model writes JSON" to "server runs shell". Defining a check takes an authenticated owner or admin, the same gate as invites; naming one that does not exist FAILS rather than passing, so an agent inventing a name gets "no check named", never execution. Two states are deliberately not failures but are recorded as `skipped` rather than quietly passing: a task that named no checks, and a workspace that is not on this machine — running the command here would test the wrong tree. A check that times out is a failure, because a check that cannot finish has not passed |
| **1.30** | No view change — the **completion gate**: a task that declares `expectedOutputs` cannot reach `completed` until an artifact of each kind is recorded, and any artifact naming a file must have that file on disk | "Done" meant "the agent said so", and for a local agent it did not even mean that: `watchForCompletion` marks a task complete after **8 seconds of terminal silence**, so an agent pausing to think for nine seconds was finished. Completion **cascades** — `getTaskDependents` unblocks the next wave and offers it out — so one unchecked "done" starts the next agent on inputs that were never produced, which is how a five-step plan yields five confident reports and no code. Three mechanisms already existed and none were connected: the planner emitted `expectedOutputs` per step (interpolated into the spec *text*, never stored), the `artifacts` table had `kind` and `file_path` and **no writer at all**, and `PROTOCOL.md` had always told agents to declare `"artifacts": {"diff": "path"}` while nothing read the field. The gate joins them, and the employee prompt now states the rule — a gate enforcing an instruction nobody was given would just be a tax. Two deliberate limits: a refusal **leaves the task untouched** rather than failing it, because an agent that wrote the code but skipped the declaration has not failed and failing would discard a run that cost real money; and after one send-back the completion is **allowed through, recorded as `task.completed_unverified`** and surfaced in the office, because a plan that asked for the wrong artifact must not send an agent back forever |
| **1.29** | Added **`AgentView.allowTools` / `denyPaths`**, and `agent.patch` now carries them | Tool policy existed in exactly ONE place: the runner's `created-agents.json`, on the machine that runs the agent — which on a shared floor is a teammate's laptop. The server kept no copy, so nothing could show you what somebody else's agent was permitted to do, the edit dialog had no field for it, and losing that file silently reverted the agent to the runner's defaults with no record of what it had been. The runner is still the sole ENFORCER (`ptyHarness` writes the scoped settings before every spawn); these are the durable copy and the thing the UI reads. **`null` and `[]` are different values and must stay different**: null means "no policy recorded, apply the machine's defaults", `[]` means "deliberately empty" — collapsing them would turn a blank form field into "this agent may use no tools at all". `agent.patch` had been in the protocol since 1.25 with no sender and no handler; it now has both, because a permissions UI the enforcer never hears about is a UI that lies |
| **1.28** | Added **`AgentView.roleId: string \| null`** — the role *definition* an agent is briefed from | A role used to be two hardcoded maps in `hivePrompt.ts` (six one-sentence briefs, six nouns), so adding a role meant editing TypeScript and redeploying, and a role could not declare what tools it should get or what "done" looks like for it. A role is now a markdown file resolved from three layers — built-in, `~/.logbridge/roles/`, then `<project>/hive/roles/` which wins — in Claude Code's subagent format, so an existing agent pack can be adapted instead of every role rewritten. **`roleId` is a second field rather than a widening of `role`**: `role` must stay the closed six-value enum because the renderer places sprites by it, and a role file with no declared category is mapped onto one of the six by `normalizeRole()`, so an arbitrary role always has a desk. Optional for the reason 1.22 spells out — every agent created before this has none, and a required field here would blank the office until each was recreated; those agents fall through to the old built-in brief, byte-identical, which is what keeps their identity fingerprint stable. Editing a role file needs no new invalidation: the body changes the prompt, the prompt changes `fingerprint()`, and `needsIdentity()` re-seeds the live session |
