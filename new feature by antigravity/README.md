...
| **20. Contract-Net Agent Communication & Live Sequence Flow** | Assignment Strategy Layer, Call for Proposals (`CFP`), agent bidding (`PROPOSE`), deterministic proposal scoring, artifact-bound peer handoffs (`DELEGATE_HANDOFF`), structured review/rework verdicts (`REVIEW_RESULT`), and real-time interactive **Live Sequence Flow Inspector**. | [CONTRACT_NET_COMMUNICATION.md](./CONTRACT_NET_COMMUNICATION.md) |
